...
//
//
// $Id: WlsScintillation.hh,v 1.21 2010-10-28 23:29:21 gum Exp $
...
#ifndef WlsScintillation_h
#define WlsScintillation_h 1
...
class WlsScintillation : public G4VRestDiscreteProcess
{
...
public: // Without description
	...
	WlsScintillation(const G4String& processName = "WlsScintillation",
                                 G4ProcessType type = fElectromagnetic);

	// WlsScintillation(const WlsScintillation &right);
...
public: // With description
        ...
        // WlsScintillation Process has both PostStepDoIt (for energy 
	...
	void SetTrackSecondariesFirst(const G4bool state);
        ...
        // If set, the WlsScintillation process expects the user to have
...
inline 
G4bool WlsScintillation::IsApplicable(const G4ParticleDefinition& aParticleType)
{
...
inline 
void WlsScintillation::SetTrackSecondariesFirst(const G4bool state) 
{ 
	fTrackSecondariesFirst = state;
}

inline
void WlsScintillation::SetFiniteRiseTime(const G4bool state)
{
        fFiniteRiseTime = state;
}

inline
G4bool WlsScintillation::GetTrackSecondariesFirst() const
{
        return fTrackSecondariesFirst;
}

inline 
G4bool WlsScintillation::GetFiniteRiseTime() const
{
        return fFiniteRiseTime;
}

inline
void WlsScintillation::SetScintillationYieldFactor(const G4double yieldfactor)
{
        YieldFactor = yieldfactor;
}

inline
G4double WlsScintillation::GetScintillationYieldFactor() const
{
        return YieldFactor;
}

inline
void WlsScintillation::SetScintillationExcitationRatio(const G4double excitationratio)
{
        ExcitationRatio = excitationratio;
}

inline
G4double WlsScintillation::GetScintillationExcitationRatio() const
{
        return ExcitationRatio;
}

inline
G4PhysicsTable* WlsScintillation::GetSlowIntegralTable() const
{
        return theSlowIntegralTable;
}

inline
G4PhysicsTable* WlsScintillation::GetFastIntegralTable() const
{
        return theFastIntegralTable;
}

inline
void WlsScintillation::DumpPhysicsTable() const
{
...
inline
G4double WlsScintillation::single_exp(G4double t, G4double tau2)
{
         return std::exp(-1.0*t/tau2)/tau2;
}

inline
G4double WlsScintillation::bi_exp(G4double t, G4double tau1, G4double tau2)
{
         return std::exp(-1.0*t/tau2)*(1-std::exp(-1.0*t/tau1))/tau2/tau2*(tau1+tau2);
}

#endif /* WlsScintillation_h */
