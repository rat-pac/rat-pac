...
# include <RAT/json_assertions.hh>
...
std::vector<int>
Value::asIntArray() const
{
   JSON_ASSERT(arrayType() == intValue);
   std::vector<int> array(size());
   
   for (unsigned int i=0; i < size(); i++)
      array[i] = (*this)[i].asInt();
      
   return array;
}

std::vector<double> 
Value::asDoubleArray() const
{
   JSON_ASSERT(arrayType() == realValue);
   std::vector<double> array(size());
   
   for (unsigned int i=0; i < size(); i++)
      array[i] = (*this)[i].asDouble();
      
   return array;   
}

std::vector<bool> 
Value::asBoolArray() const
{
   JSON_ASSERT(arrayType() == booleanValue);
   std::vector<bool> array(size());
   
   for (unsigned int i=0; i < size(); i++)
      array[i] = (*this)[i].asBool();
      
   return array;
}

std::vector<std::string>
Value::asStringArray() const
{
   JSON_ASSERT(arrayType() == stringValue);
   std::vector<std::string> array(size());
   
   for (unsigned int i=0; i < size(); i++)
      array[i] = (*this)[i].asString();
...
Value::isTypedArray() const
{
   ...
   ValueType type = (*this)[(unsigned int)0].type();
...
{
   JSON_ASSERT(isTypedArray());

   return (*this)[(unsigned int)0].type();
}
