...
CubicPMTConstruction::CubicPMTConstruction(DBLinkPtr table, G4LogicalVolume *mother) : PMTConstruction("toroidal") {
  ...
  G4VSolid *CubicPMTConstruction::NewEnvelopeSolid(const std::string &name)
  {
  ...
  void CubicPMTConstruction::CalcInnerParams(GLG4TorusStack *body,
                                  ...
                                  double &zLowestDynode)
  {
    ...
    if (equatorIndex < 0)
      Log::Die("CubicPMTConstruction::CalcInnerParams: Pathological PMT shape with no equator edge");
    // sanity check on dynode height
    if (fParams.dynodeTop > innerZEdge[equatorIndex])
      Log::Die("CubicPMTConstruction::CalcInnerParams: Top of PMT dynode cannot be higher than equator.");
  }
