...
namespace RAT {
  ...
  GeoPMTParser::GeoPMTParser(DBLinkPtr table, bool useSD)
  {
    ...
    try {
      fParam.photocathode_MINrho = lpmt->GetD("photocathode_MINrho");
      fParam.photocathode_MAXrho = lpmt->GetD("photocathode_MAXrho");
    }
    catch (DBNotFoundError& e) {
      fParam.photocathode_MINrho = 0.;// Need to change to max RHO
      fParam.photocathode_MAXrho = 0.;// Need to change to max RHO
    }
