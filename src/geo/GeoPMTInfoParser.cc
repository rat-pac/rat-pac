...
GeoPMTInfoParser::GeoPMTInfoParser(DBLinkPtr lpos_table, const std::string &mother_name) {  
    ...
    try {
        ...
        Log::Assert(pos_x.size() == pos_y.size() && pos_x.size() == pos_z.size(),"GeoPMTInfoParser: PMTINFO arrays must be same length!");
        fPos.resize(pos_x.size());
        for (size_t i = 0; i < pos_x.size(); i++) {
            fPos[i].set(pos_x[i],pos_y[i],pos_z[i]);
        }
    } catch (DBNotFoundError &e) {
        Log::Die("GeoPMTInfoParser: PMTINFO does not specify position information");
    }
    ...
    try {
        ...
        Log::Assert(fPos.size() == dir_x.size() && fPos.size() == dir_y.size() && fPos.size() == dir_z.size(),"GeoPMTInfoParser: PMTINFO arrays must be same length!");
        ...
        warn << "GeoPMTInfoParser: PMTINFO does not specify direction information" << endl;
        fDir.resize(0);
    }
    ...
    try {
        ...
        Log::Assert(fPos.size() == fType.size(),"GeoPMTInfoParser: PMTINFO arrays must be same length!");
    } catch (DBNotFoundError &e) {
    ...
    try {
        ...
        Log::Assert(fPos.size() == fEfficiencyCorrection.size(),"GeoPMTInfoParser: PMTINFO arrays must be same length!");
    } catch (DBNotFoundError &e) {
    ...
    if (log_mother == 0)
        Log::Die("GeoPMTParser: PMT mother logical volume " + mother_name + " not found");
  ...
  G4RotationMatrix GeoPMTInfoParser::GetPMTRotation(int i) const {
