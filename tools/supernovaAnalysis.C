...
void supernovaAnalysis(const char *file) {
    ...
    TH1D *hCos0FB_SN = new TH1D("hCos0FB_SN","primary event",11,-1.0,1.0);
    hCos0FB_SN->SetLineColor(4);
    TH1D *hCos1FB_SN = new TH1D("hCos1FB_SN","secondary event",11,-1,1);
    ...
    for (int i = 0; i < nEvents; i++) {
        ...
        Int_t nTriggers = MCS->GetNPhotonsTriggers();
        printf("num of triggers: %d\n",nTriggers);
        for(int ii =0; ii< nTriggers;ii++){
			printf(" %6.3e %7d %7d %7d\n",MCS->GetTimePhotons(ii),MCS->GetCherenkovPhotons(ii),
			MCS->GetScintPhotons(ii),MCS->GetRemmitPhotons(ii));
        }
